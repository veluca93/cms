...
class ScoringService(Service):
        ...
        with SessionGen(commit=False) as session:
            ...
            for submission in contest.get_submissions():
                for dataset in get_autojudge_datasets(submission.task):
                    ...
                    if (sr.evaluated() or sr.compilation_outcome == "fail") \
                            and sr_id not in self.submission_results_scored:
                        new_submission_results_to_score.add(sr_id)

                if submission.tokened() and submission.id not in \
                        self.submissions_tokened:
    ...
    def invalidate_submission(self,
        ...
        old_s = len(self.submission_results_to_score)
        old_t = len(self.submission_results_to_token)
        self.submission_results_to_score |= new_submission_results_to_score
        if old_s + old_t == 0:
            self.add_timeout(self.score_old_submissions, None,
                             0.5, immediately=False)
